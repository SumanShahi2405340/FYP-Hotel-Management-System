...
export default function OwnerForgotPasswordForm() {
  ...
  const requestOTP = async (e) => {
    e.preventDefault();
    try {
      const res = await api.post('/owner/forgot-password/', { email });  // 
      setMessage(res.data.message || 'OTP sent to email');
      router.push(`/owner/verify-otp?email=${email}`);  // 
    } catch (err) {
  ...
  return (
    <form onSubmit={requestOTP} className="max-w-md mx-auto p-6 border rounded">
      <h2 className="text-xl font-bold mb-4">Owner Forgot Password</h2>
      <input
        type="email"
        placeholder="Email"
        value={email}
