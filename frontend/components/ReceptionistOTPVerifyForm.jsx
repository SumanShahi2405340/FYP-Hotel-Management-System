...
import { FaArrowLeft, FaCheckCircle, FaKey, FaShieldAlt, FaSpinner } from 'react-icons/fa';

export default function ReceptionistOTPVerifyForm() {
  ...
  useEffect(() => {
    if (!email) {
      router.push('/receptionist/forgot-password');
    }
  }, [email, router]);
  ...
  const handleVerifyOTP = async (e) => {
    ...
    if (!email) {
      setMessage('Email is missing. Please request OTP again.');
    ...
    try {
      const res = await recepApi.post('/api/receptionist/verify-otp/', {
        email,
        otp,
      });
      ...
      setTimeout(() => {
        router.push(`/receptionist/reset-password?email=${encodeURIComponent(email)}`);
      }, 1200);
    } catch (err) {
      ...
      <div className="relative z-20 w-full max-w-md mx-4">
        <div className="relative bg-white/5 backdrop-blur-xl rounded-2xl shadow-2xl p-8 border border-white/10 hover:border-amber-500/30 transition-all duration-500">
          ...
          <div className="mt-6 text-center">
            <Link href="/receptionist/forgot-password" className="inline-flex items-center gap-2 text-sm text-gray-400 hover:text-amber-400 transition-colors">
