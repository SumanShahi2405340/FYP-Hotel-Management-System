...
import { FaLock, FaArrowLeft, FaCheckCircle, FaSpinner, FaEye, FaEyeSlash } from 'react-icons/fa';
...
export default function AdminResetPassword() {
  ...
  const handleResetPassword = async (e) => {
    ...
    try {
      const res = await api.post('/api/update-password/', {
        email,
      ...
      <div className="relative z-20 w-full max-w-md mx-4">
        <div className="relative bg-white/5 backdrop-blur-xl rounded-2xl shadow-2xl p-8 border border-white/10 hover:border-amber-500/30 transition-all duration-500">
          ...
          <div className="mt-6 text-center">
            <Link href="/admin/login" className="inline-flex items-center gap-2 text-sm text-gray-400 hover:text-amber-400 transition-colors">
