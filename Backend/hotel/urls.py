...
urlpatterns = [
    ...
    path("owner-send-announcement/", owner_send_announcement, name="owner-send-announcement"),
   ...
   # Starred Notifications endpoints (update these in urlpatterns)
    ...
    # Hotel info API (contains hotel_id and hotel_name retrieved from token)
    path("me/", me, name='me'),
    ...
    # Owner Authentication endpoints (Forgot Password, OTP Verify, Update Password)
    path('owner/forgot-password/', OwnerOTPRequestView.as_view(), name='owner-forgot-password'),
    path('owner/verify-otp/', OwnerOTPVerifyView.as_view(), name='owner-verify-otp'),
    path('owner/update-password/', OwnerUpdatePasswordView.as_view(), name='owner-update-password'),

    # ============================================================
    # ROOM IMAGES MANAGEMENT ENDPOINTS
    # ============================================================
    # GET: Get images for specific room or all rooms
    # POST: Upload images for a room (replaces existing)
    # DELETE: Delete all images for a room
    path('room-images/', RoomImagesView.as_view(), name='room-images'),
    
    # GET/DELETE/PATCH: Manage individual room image by ID
    path('room-images/<int:pk>/', SingleRoomImageView.as_view(), name='room-image-detail'),
    ...
    path('guest/forgot-password/', GuestForgotPasswordView.as_view(), name='guest-forgot-password'),
    path('guest/verify-otp/', GuestVerifyOTPView.as_view(), name='guest-verify-otp'),
    path('guest/update-password/', GuestUpdatePasswordView.as_view(), name='guest-update-password'),


    # Guest JWT Token Refresh
    path('guest/token/refresh/', GuestTokenRefreshView.as_view(), name='guest-token-refresh'),


    # Guest room availability endpoints
    path('hotels/<int:hotel_id>/guest-rooms/', get_hotel_rooms_for_guest, name='guest-hotel-rooms'),
    path('hotels/<int:hotel_id>/guest-bookings/', get_hotel_bookings_for_guest, name='guest-hotel-bookings'),
        
    # Add these after your existing guest URLs (around line 200)

    # # Guest reviews endpoint
    # path('hotels/<int:hotel_id>/guest-reviews/', get_hotel_reviews_for_guest, name='guest-hotel-reviews'),

    # Guest rooms list endpoint (if you need a simpler version)
    path('hotels/<int:hotel_id>/guest-rooms-list/', get_hotel_rooms_for_guest, name='guest-hotel-rooms-list'),    



    # Include router URLs (for HotelViewSet, ManageBookingsViewSet, etc.)
